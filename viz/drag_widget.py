...
class DragWidget:
    def __init__(self, viz):
        ...
        self.tracker_type   = 'nearest'  # 'nearest', 'simple_raft', 'pips', 'raft'
    ...
    def __call__(self, show=True):
        ...
        if show:
            with imgui_utils.grayed_out(self.disabled_time != 0):
                ...
                imgui.same_line(viz.label_w)
                imgui.text('Tracker')
                imgui.same_line(viz.label_w + 50)
                
                tracker_options = ['nearest', 'simple_raft', 'pips', 'raft']
                current_index = tracker_options.index(self.tracker_type) if self.tracker_type in tracker_options else 0
                
                changed, new_index = imgui.combo("##tracker_select", current_index, tracker_options)
                if changed:
                    self.tracker_type = tracker_options[new_index]
        ...
        viz.args.r1 = self.r1
        viz.args.r2 = self.r2
        viz.args.tracker_type = self.tracker_type  # Pass tracker type to renderer
